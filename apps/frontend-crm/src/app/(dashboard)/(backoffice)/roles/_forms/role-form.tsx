...
import { useRoleForm } from "../_form-hooks/use-role-form";
...
export function RoleForm({ role, onSuccess }: RoleFormProps) {
  ...
  const { form, mode } = useRoleForm({
    mode: role ? "edit" : "create",
    initialData: role,
  });

  const onSubmit = (data: CreateRoleInputType | UpdateRoleInputType) => {
    if (mode === "edit" && role) {
      updateRole.mutate(
        {
          params: { id: role.id },
          body: data as UpdateRoleInputType,
        },
        {
          onSuccess: () => {
            onSuccess?.();
      ...
      createRole.mutate(
        { body: data as CreateRoleInputType },
        {
          onSuccess: () => {
            onSuccess?.();
          },
