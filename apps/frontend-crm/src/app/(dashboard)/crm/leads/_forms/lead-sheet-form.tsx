...
import { CreateLeadInputType, LeadOutput } from "@workspace/orpc-contract";
import { z } from "zod";
...
export const LeadSheetForm = ({
  ...
  const onSubmit = async (data: CreateLeadInputType) => {
    const cleanData = {
      ...data,
    ...
    try {
      if (mode === "create") {
        await createLead.mutateAsync({ body: cleanData });
